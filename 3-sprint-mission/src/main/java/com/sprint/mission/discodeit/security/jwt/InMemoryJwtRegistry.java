...
public class InMemoryJwtRegistry implements JwtRegistry {
    ...
    public void rotateJwtInformation(String refreshToken, JwtInformation newJwtInformation) {
        origin.computeIfPresent(
                newJwtInformation.getUserDto().id(),
                (key, queue) -> {
                    queue.stream().filter(
                                    jwtInformation ->
                                            jwtInformation.getAccessToken().equals(refreshToken)
                            )
                            .findFirst()
                            .ifPresent(
                                    jwtInformation -> {
    ...
    @Scheduled(fixedDelay = 1000 * 60 * 5) // 1000 (ms) * 60 (sec) * 5 (min)
    @Override
    ...
    public void addTokenIndex(String accessToken, String refreshToken) {
        accessTokenIndexes.add(accessToken);
        refreshTokenIndexes.add(refreshToken);
    }

    public void removeTokenIndex(String accessToken, String refreshToken) {
