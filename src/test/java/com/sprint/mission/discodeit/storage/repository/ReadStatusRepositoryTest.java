package com.sprint.mission.discodeit.storage.repository;
...
class ReadStatusRepositoryTest {
   ...
   */
  private ReadStatus createTestReadStatus(User user, Channel channel, boolean notificationEnabled, Instant lastReadAt) {
    ReadStatus readStatus = new ReadStatus(user, channel, notificationEnabled, lastReadAt);
  ...
  void findAllByUserId_ReturnsReadStatuses() {
    ...
    ReadStatus readStatus1 = createTestReadStatus(user, channel1, notificationEnabled, now.minus(1, ChronoUnit.DAYS));
    ReadStatus readStatus2 = createTestReadStatus(user, channel2, notificationEnabled, now);
  ...
  void findAllByChannelIdWithUser_ReturnsReadStatusesWithUser() {
    ...
    Instant now = Instant.now();
    ReadStatus readStatus1 = createTestReadStatus(user1, channel, notificationEnabled, now.minus(1, ChronoUnit.DAYS));
    ReadStatus readStatus2 = createTestReadStatus(user2, channel, notificationEnabled, now);
  ...
  void existsByUserIdAndChannelId_ExistingStatus_ReturnsTrue() {
    ...
    Channel channel = createTestChannel(ChannelType.PUBLIC, "공개채널");
    boolean notificationEnabled = true;
    ReadStatus readStatus = createTestReadStatus(user, channel, notificationEnabled, Instant.now());
  ...
  void deleteAllByChannelId_DeletesAllReadStatuses() {
    ...
    boolean notificationEnabled = true;
    // 삭제할 채널에 읽음 상태 2개 생성
    createTestReadStatus(user1, channel, notificationEnabled, Instant.now());
    createTestReadStatus(user2, channel, notificationEnabled, Instant.now());

    // 유지할 채널에 읽음 상태 1개 생성
    createTestReadStatus(user1, otherChannel, notificationEnabled, Instant.now());
