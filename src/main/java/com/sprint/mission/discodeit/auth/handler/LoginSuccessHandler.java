...
public class LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        if (authentication.getPrincipal() instanceof DiscodeitUserDetails discodeitUserDetails) {
            UserResponseDto userResponseDto = discodeitUserDetails.getUserResponseDto();
            ...
            String responseBody = objectMapper.writeValueAsString(userResponseDto);
            response.getWriter().write(responseBody);

            log.debug("[LoginSuccessHandler] 로그인 성공 응답 완료: {}", userResponseDto.username());
        } else {
