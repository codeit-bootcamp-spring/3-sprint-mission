...
public interface UserApi {
    ...
    ResponseEntity<UserResponse> create(
            ...
            @Parameter(description = "User 프로필 이미지", content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE)) MultipartFile profile);

    @Operation(summary = "User 정보 수정")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User 정보가 성공적으로 수정됨", content = @Content(schema = @Schema(implementation = UserResponse.class))),
    ...
    @Operation(summary = "User의 상태 정보 수정")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "UserStatus가 성공적으로 수정됨", content = @Content(schema = @Schema(implementation = UserStatusResponse.class))),
            @ApiResponse(responseCode = "404", description = "User 또는 UserStatus를 찾을 수 없음", content = @Content(examples = @ExampleObject("UserStatus with userId {userId} not found")))
    })
    ResponseEntity<UserStatusResponse> updateUserStatusByUserId(
            @Parameter(description = "상태를 수정할 User ID") UUID userId,
            @Parameter(description = "수정할 UserStatus 정보") UserStatusUpdateRequest request);
}
