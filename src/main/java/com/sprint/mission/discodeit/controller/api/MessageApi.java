...
public interface MessageApi {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "Channel 또는 Author를 찾을 수 없음", content = @Content(examples = @ExampleObject("Channel with id {channelId} does not exist")))
    })
    ResponseEntity<MessageResponse> create(
            ...
            @Parameter(description = "첨부파일 목록", content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE)) List<MultipartFile> attachments);

    @Operation(summary = "Message 내용 수정")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Message 내용이 성공적으로 수정됨", content = @Content(schema = @Schema(implementation = MessageResponse.class))),
    ...
    @Operation(summary = "Channel의 모든 Message 조회")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Message 목록 조회 성공", content = @Content(array = @ArraySchema(schema = @Schema(implementation = MessageResponse.class))))
    })
