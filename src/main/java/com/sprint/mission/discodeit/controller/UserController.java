...
public class UserController {
  ...
  @ApiResponses(value = {
      ...
      @ApiResponse(responseCode = "400", description = "같은 newEmail 또는 username을 사용하는 User가 이미 존재함", content = @Content(schema = @Schema(example = "User with newEmail {newEmail} already exists")))
  })
  @PostMapping(value = "/users", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
  public ResponseEntity<User> create(
      @RequestPart("userCreateRequest") UserCreateRequest createUserRequest,
  ...
  @Parameter(name = "userId", description = "수정할 User ID", required = true, schema = @Schema(type = "string", format = "uuid"))
  ...
  @Parameter(name = "userId", description = "삭제할 User ID", required = true, schema = @Schema(type = "string", format = "uuid"))
  ...
  @Parameter(name = "userId", description = "상태를 변경할 UserID")
