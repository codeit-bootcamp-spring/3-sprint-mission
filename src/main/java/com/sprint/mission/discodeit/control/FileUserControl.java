            ...
            switch (choice) {
                ...
                case 2:
                    while (true){                    // 1_4_2 사용자 이름 변경 메뉴
                        ...
                        User user = fuserService.findUserByName(newName);
                        if(user != null){             // 중복값 존재시 재입력
                            System.out.print(" ▶ 이미 존재하는 이름입니다. 다시 입력해 주세요.\n >> ");
                        } else {                       // 중복값 미확인시 유저명 변경
                            fuserService.updateUserName(currentUser, newName);
                            break;
                        }
                    }
                    break;
                case 3:                    // 1_4_3 사용자 삭제 메뉴
                    ...
                    fuserService.showAllUsers();
                    System.out.print(" >> ");
                    String deleteUser = scanner.nextLine();
                    if ((deleteUser.isEmpty()) || (fuserService.findUserByName(deleteUser) == null)) {
                        ...
                        User userToDel = fuserService.findUserByName(deleteUser);
                        ...
                        if (deleteConfirm.isEmpty()) {
                            ...
                            fuserService.deleteUser(deleteUser);
                ...
                case 4:                    // 1_4_4 단일 사용자 정보 출력 메뉴
                    ...
                    if ((oneUser.isEmpty()) || (fuserService.findUserByName(oneUser) == null)) {
                        System.out.println(" ▶ 존재하지 않는 사용자이거나 잘못된 입력입니다. 이전 메뉴로 돌아갑니다.");
                    } else {
                        System.out.println(" ▶ 입력하신 사용자는 아래와 같습니다.");
                        User userToRead = fuserService.findUserByName(oneUser);
                ...
                case 6:                    // 1_4_6 다른 사용자로 진행
                    ...
                    if (user != null) {
                        ...
                        fuserService.getUserslist().forEach(u -> System.out.print(u.getName()+ " , "));
                        System.out.println();
                    }
                    break;
