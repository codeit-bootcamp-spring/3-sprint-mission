...
public class BasicChannelService implements ChannelService {
  ...
  public Channel create(PublicChannelCreateRequest publicChannelCreateRequest) {
    Channel channel = new Channel(ChannelType.PUBLIC, publicChannelCreateRequest.ownerId(),
  ...
  public Channel create(PrivateChannelCreateRequest privateCreateRequest) {
    ...
    if (privateCreateRequest.ownerId() != null) {
      this.readStatusRepository.save(
          new ReadStatus(privateCreateRequest.ownerId(), channel.getId(), channel.getCreatedAt()));
    }
    /* participantIds 에 있는 유저들 ReadStatus 생성 */
    for (UUID attendeeId : privateCreateRequest.participantIds()) {
      this.readStatusRepository.save(
          new ReadStatus(attendeeId, channel.getId(), channel.getCreatedAt()));
    }
  ...
  private ChannelDto toDto(Channel channel) {
    ...
    if (channel.getType().equals(ChannelType.PRIVATE)) {
      participantIds = this.findParticipantIdsByChannel(channel.getId());
    }

    return new ChannelDto(channel.getId(), channel.getType(), channel.getName(),
        channel.getDescription(), channel.getOwnerId(), participantIds, channel.getLastMessageAt());
  }
