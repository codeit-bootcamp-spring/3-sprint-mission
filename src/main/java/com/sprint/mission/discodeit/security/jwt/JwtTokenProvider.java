...
public class JwtTokenProvider {
    ...
    private String generateToken(
        ...
        List<String> expandedRoles = userDetails.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .flatMap(auth -> {
                    if ("ROLE_ADMIN".equals(auth)) {
                        return List.of("ROLE_ADMIN", "ROLE_CHANNEL_MANAGER", "ROLE_USER").stream();
                    } else if ("ROLE_CHANNEL_MANAGER".equals(auth)) {
                        return List.of("ROLE_CHANNEL_MANAGER", "ROLE_USER").stream();
                    } else {
                        return List.of(auth).stream();
                    }
                })
                .distinct()
        ...
        log.debug("Generated {} token for user: {}, roles={}", tokenType, user.username(), expandedRoles);
        return token;
    }
