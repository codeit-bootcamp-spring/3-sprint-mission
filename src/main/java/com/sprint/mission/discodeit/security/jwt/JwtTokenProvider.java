...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(
        ...
        log.info("Loaded ACCESS_TOKEN_SECRET length: {}", accessTokenSecret.length());
        log.info("Loaded REFRESH_TOKEN_SECRET length: {}", refreshTokenSecret.length());

        this.accessTokenExpirationMs = accessTokenExpirationMs;
        this.refreshTokenExpirationMs = refreshTokenExpirationMs;

        byte[] accessSecretBytes = Base64.getUrlDecoder().decode(accessTokenSecret);
        this.accessTokenSigner = new MACSigner(accessSecretBytes);
        this.accessTokenVerifier = new MACVerifier(accessSecretBytes);

        byte[] refreshSecretBytes = Base64.getUrlDecoder().decode(refreshTokenSecret);
