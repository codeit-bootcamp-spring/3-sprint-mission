...
import com.sprint.mission.discodeit.exception.redis.RedisLockAcquisitionException;
...
public class RedisLockProvider {
    ...
    public void acquireLock(String key) {
        ...
        if (Boolean.TRUE.equals(acquired)) {
            log.debug("[RedisLockProvider] 분산 락 획득 성공: {} (값: {})", lockKey, lockValue);
        } else {
            log.debug("[RedisLockProvider] 분산 락 획득 실패: {}", lockKey);
    ...
    public void releaseLock(String key) {
        ...
        try {
            ...
            log.debug("[RedisLockProvider] 분산 락 해제 완료: {}", lockKey);
        } catch (Exception e) {
