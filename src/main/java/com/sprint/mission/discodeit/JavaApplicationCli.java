...
import com.sprint.mission.discodeit.cli.ChannelCli;
 ...
 */
public class JavaApplicationCli {
    ...
    private static void  singleMessageSearchTest() {
        ...
        channel.getMessages().forEach(message -> System.out.println("메세지ID : "+message.getId()));
        System.out.println("입력 : ");
        String choice = sc.nextLine();
        if(!choice.equals("0")) {
            UUID inputMessageId = UUID.fromString(choice);
            Message selectedMessage = messageService.findMessageByMessageId(inputMessageId);
    ...
    private static void singleChannelSearchTest(){
        ...
        if(!choice.equals("0")) {
            try{
                ...
                Channel selectedChannel = channelService.findChannelById(inputChannelId);
